...
public class Library {
    ...
    // Location of all resources for internationalization.
    public static String resourceDirectory = libraryDirectory + "Resources/";
     // Location of the Help Files under internationalization.
    public static String helpDirectory;
    // International text for building menus, messages, and screens
    public static MyResBundle menuItems, messages, screenElements;
     ...
     * SILK files. It functions as a pushdown stack. */
...
//        System.out.println(screenElements.getString("libDirIs") + ": " + tempParent + "/Library/");
        return tempParent + "/Library/";
    }

    static void setMenuLanguage() {
        // Set menu language, reset MainPane title
        try {
            if (menuLanguage.isEmpty()) {
                SIL_Edit.getUserLanguage();
            }
            userLocale = new Locale(menuLanguage);
            helpDirectory = resourceDirectory + "HelpFiles_" + menuLanguage + "/";
            menuItems = MyResBundle.getBundle("MenuItems", userLocale, resourceDirectory);
            messages = MyResBundle.getBundle("Messages", userLocale, resourceDirectory);
            screenElements = MyResBundle.getBundle("ScreenElements", userLocale, resourceDirectory);
            String windowTitle = screenElements.getString("kinshipAnalysis");
            if (SILKin.mainPane != null) {
                SILKin.mainPane.setTitle("SILKin: " + windowTitle);
            }            
        } catch (Exception exc) {
            ...
            String msg1, msg2;
            if (messages == null) {
                msg1 = "Could not find .properties file";
                msg2 = "Fatal Error";
            }else {
                msg1 = messages.getString("missingRes");
                msg2 = messages.getString("seriousError");
            }
            JOptionPane.showMessageDialog(aBox, 
                    msg1 + exc, msg2, JOptionPane.ERROR_MESSAGE);        
            System.exit(7);
        }
    }
    ...
    public static void readStubFile() throws JavaSystemException {
        String fileName = libraryDirectory + "Contexts.stub", line, menuLang;
        int num, comma, start = 0, size;
        BufferedReader file;
        try {
            file = new BufferedReader(new FileReader(fileName));
        } catch (FileNotFoundException e) {
            throw new JavaSystemException(messages.getString("noStubFile") + e);
        }
        try {
            line = file.readLine();
            ...
            start = line.indexOf("=");
            if (start == -1) {
                throw new JavaSystemException(messages.getString("stubFileBad"));
            }
            // Read optional menu language specification
            menuLang = line.substring(0, start);
            if (menuLang.equals("menuLang")) {
                if (line.length() > ++start) {
                    menuLanguage = line.substring(start);
                }
                line = file.readLine();
                start = line.indexOf("=");
                if (start == -1) {
            ...
            throw new JavaSystemException(messages.getString("stubOrIndexBad") + " " + e);
        } catch (FileNotFoundException fnf) {
            throw new JavaSystemException(messages.getString("stubOrIndexBad") + " "  + fnf);
        } catch (IOException io) {
            throw new JavaSystemException(messages.getString("stubOrIndexBad") + " "  + io);
        }
    ...
    public static void chooseUserContext(KSJInternalFrame window) throws JavaSystemException {
        ...
        if ((contextUnderConstruction != null) && contextUnderConstruction.saveState) {
            String msg = messages.getString("ctxtOpen1")
                    + " '" + contextUnderConstruction.languageName + "' " 
                    + messages.getString("ctxtOpen2");
            String btn1 = messages.getString("saveData1st"),
                   btn2 = messages.getString("closeIt");
            Object[] options = { btn1, btn2 };
            int choice = JOptionPane.showOptionDialog(window, msg, messages.getString("savePrior?"),
            ...
            if (choice == 0) {
                try {
                    ...
                    throw new JavaSystemException(messages.getString("whileSvgCtxt") + " '"
                            + contextUnderConstruction.languageName + "'\n" + ex);
                }
            }
        ...
        fc.setDialogTitle(messages.getString("chooseCtxtFile"));
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                throw new JavaSystemException(messages.getString("fileNotFound") + ":\n" + e);
            ...
            try {
                ...
                throw new JavaSystemException(messages.getString("filepathProblem") + ":\n" + iox);
            }
        } //  end of if-returnVal=Approve
        else {  // if they cancel out
            String msg = messages.getString("needCtxt");
            String btn1 = messages.getString("chooseExisting"),
                   btn2 = messages.getString("createNew"),
                   btn3 = messages.getString("closeDataWindow");
            Object[] options = {btn1, btn2, btn3};
            int choice = JOptionPane.showOptionDialog(window, msg, messages.getString("dataNeedsCtxt"),
    ...
    public static void createUserContext(KSJInternalFrame window) throws JavaSystemException {
        ...
        String msg = messages.getString("ctxtName1") + messages.getString("ctxtName2");
        ...
        while (!validateFileName(langName, false)) {
            msg = messages.getString("ctxtName1")
                    + messages.getString("theName") + " '" + langName + "' "
                    + messages.getString("violates")
                    + messages.getString("ctxtName2");
            msg += messages.getString("ctxtName3");
            msg += messages.getString("ctxtName4");
            msg += messages.getString("ctxtName5");
            langName = JOptionPane.showInputDialog(msg);
        }  //  end of harrass-em-until-they-give-a-good-name
        msg = messages.getString("authorName");
        author = JOptionPane.showInputDialog(msg);
        msg = messages.getString("defaultIsRef");
        JOptionPane.showMessageDialog(window, msg);
        msg = messages.getString("polygamy?");
        String btn1 = messages.getString("notAllowed"),
               btn2 = messages.getString("permitted");
        String[] options = { btn1, btn2 };
        int choice = JOptionPane.showOptionDialog(window, msg, messages.getString("equalPoly"),
                JOptionPane.YES_NO_OPTION,
                JOptionPane.QUESTION_MESSAGE,
                null, //don't use a custom Icon
                options, //the titles of buttons
        ...
        while (redo) {
            ...
            returnVal = chooseDirectory(window, messages.getString("chooseCtxtDir"));
            if (returnVal == JFileChooser.APPROVE_OPTION) {
                try {
                    ...
                    if (!writeIt) {
                        msg = messages.getString("theFile") + " " + fileName 
                                + " " + messages.getString("alreadyExists");
                        options[0] = messages.getString("overWrite");
                        options[1] = messages.getString("cancel");
                        choice = JOptionPane.showOptionDialog(window, msg, 
                                messages.getString("fileOverWrite"),
                                JOptionPane.YES_NO_OPTION,
                                JOptionPane.QUESTION_MESSAGE,
                                null, //don't use a custom Icon
                                options, //the titles of buttons
                    ...
                    if (writeIt) {
                        ...
                        currentActivity = DATA_GATHERING;
                        msg = messages.getString("ctxtSave1") + " " + langName + ", "
                              + messages.getString("ctxtSave2") + " " 
                                + userDirectory + messages.getString("ctxtSave3");
                        JOptionPane.showMessageDialog(window, msg);
                    } // end of Write-It
                } catch (FileNotFoundException fnf) {
                    throw new JavaSystemException(messages.getString("whileSvgCtxt") + ": " 
                    ...
                    throw new JavaSystemException(messages.getString("whileSvgCtxt") + ": " + fileName + ":\n" + ix);
                }

            } //  end of User-Chose-A-Directory
            else {  // They canceled out.  BUMMER.  Erase everything.
                msg = messages.getString("ctxtCancel1") + " '" + langName + "', "
                        + messages.getString("ctxtCancel2") + " " 
                        + menuItems.getString("file") + " > " + menuItems.getString("new")
                        + " > " + menuItems.getString("userContext") 
                        + messages.getString("ctxtCancel3");
                JOptionPane.showMessageDialog(window, msg);
    ...
    public static int chooseDirectory(KSJInternalFrame window, String title) {
        ...
        fc.setApproveButtonText(messages.getString("chooseDirectory"));
...
//        }  //  end of loop thru characters in the fileName
    ...
    public static void saveUserContext(KSJInternalFrame frame, boolean newName)
        ...
        msg = messages.getString("ctxtFormatOptions");
        String[] options = {".silk", ".ctxt"};
        int formatChoice = JOptionPane.showOptionDialog(frame, msg, 
                messages.getString("chooseFileFormat"),
        ...
        if (!newName) {  //  save-as-is
            ...
            fc.setFileSelectionMode(JFileChooser.FILES_AND_DIRECTORIES);
            fc.setDialogTitle(messages.getString("chooseCtxtDir"));
            fc.setApproveButtonText(messages.getString("saveContextAs"));
            ...
            while (redo) {
                ...
                if (returnVal == JFileChooser.APPROVE_OPTION) {
                    ...
                    writeIt = !trial.exists();
                    if (!writeIt) {
                        msg = messages.getString("theFile") + "\n" + filePath + "\n"
                                + messages.getString("alreadyExists.");
                        options[0] = messages.getString("overWrite");
                        options[1] = messages.getString("cancel");
                        int choice = JOptionPane.showOptionDialog(frame, msg, 
                                messages.getString("fileOverWrite"),
                                JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE,
                    ...
                    if (writeIt) {
                        ...
                        if (!fileName.equals(contextUnderConstruction.languageName)) {
                            msg = messages.getString("ctxtNameCk") + " " + fileName + "?";
                            options[0] = fileName;
                            options[1] = contextUnderConstruction.languageName;
                            int choice = JOptionPane.showOptionDialog(frame, msg, 
                                    messages.getString("confirmName"),
                                    JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE,
                                    null, options, options[0]);
                            if (choice == 0) {
                        ...
                        userContextName = contextUnderConstruction.languageName;
                        msg = fileName + " " + messages.getString("isNewCtxt");
                        JOptionPane.showMessageDialog(frame, msg,
                                messages.getString("changingTheCtxt"),
    ...
    public static void writeStubFile() throws JavaSystemException {
        ...
        try {
            ...
            outFile.println("menuLang=" + menuLanguage);
            ...
            throw new JavaSystemException(messages.getString("stubCreationError") + e);
        } //  end of catch block
    }  //  end of method writeStubFile

    /** Make a new version of the Library's ArrayList<ContextStub> (stubs) which 
     * omits this stub. Also delete this context from ActiveContexts, if it is there.
     * 
    ...
    static TreeMap readKTSig(BufferedReader file) throws JavaSystemException, IOException {
        ...
        while (line != null) {
            ...
            if (end == -1) {
                throw new JavaSystemException(messages.getString("corruptedKTSig"));
            }
    ...
    static void writePredEncodings(String suffix) throws JavaSystemException {
        ...
        try {
            ...
            throw new JavaSystemException(messages.getString("predEncodingsFileCreationError") + e);
        } //  end of catch block
    ...
    static void readPredEncodings() throws JavaSystemException {
        try {
            ...
            throw new JavaSystemException(messages.getString("predEncodingsFileCreationError") + io);
        }
    }
    ...
    static TreeMap readPredEncodings(BufferedReader file) throws JavaSystemException {
        ...
        try {
            ...
            throw new JavaSystemException(messages.getString("predEncodingsFileReadError") + e);
        } catch (IndexOutOfBoundsException ioob) {
            throw new JavaSystemException(messages.getString("predEncodingsFileReadError") + ioob);
        } catch (IOException io) {
            throw new JavaSystemException(messages.getString("predEncodingsFileReadError") + io);
        }
        return tMap;
    ...
    static void writePredDecodings(String suffix) throws JavaSystemException {
        ...
        try {
            ...
            throw new JavaSystemException(messages.getString("predDecodingsFileCreationError") + e);
        } //  end of catch block
    ...
    static void readPredDecodings() throws JavaSystemException {
        try {
            ...
            throw new JavaSystemException(messages.getString("predDecodingsFileReadError") + io);
        }
    }
    ...
    static TreeMap readPredDecodings(BufferedReader file) throws JavaSystemException {
        ...
        try {
            ...
            throw new JavaSystemException(messages.getString("predDecodingsFileReadError") + e);
        } catch (IndexOutOfBoundsException ioob) {
            throw new JavaSystemException(messages.getString("predDecodingsFileReadError") + ioob);
        } catch (IOException io) {
            throw new JavaSystemException(messages.getString("predDecodingsFileReadError") + io);
        }
        return tMap;
     ...
     */
    public static class KTD_Coder implements Serializable {
    ...
    /**
     * The Library's <code>KTD_EQC</code> class defines equivalence classes for
     * {@link KinTermDef}s. Two KTDs are equivalent if they have the same
     * Signature String (exact) and if they have an identical set of positive
     * examples.
     * <p>
     * Instances of KTD_EQC are stored on the Library's <code>ktSigTree</code>,
     ...
     *                  garymorris2245@verizon.net
     */
    public static class KTD_EQC implements Comparable, Serializable {
        ...
        public KTD_EQC(String image, String sigStr) throws JavaSystemException {  //  for use in reading a file
            ...
            while (start > -1) {
                ...
                if (end == 0) {
                    throw new JavaSystemException(messages.getString("ktdFileError"));
                }
     ...
     * the ClauseIndex). It is used as a compact stand-in for a KinTermDef in
     * set operations during learning. The slot for 'ktd' is used to cache the
     * real KTD during construction. But when a KTD_Ptr is written to disk, the
     * ktd is not (to conserve space); it is retrieved on demand.
     */
    public static class KTD_Ptr implements Comparable, Serializable {
         ...
         */
        public static void clearCacheDTs() throws KSInternalErrorException {
            if (cache.isEmpty()) {
                return;
            }
    ...
    /** The Library's <code>ClauseCounts</code> class is a TreeMap with structure: 
     * LangName:kinTerm => Counter[] (cumulative counts of string length). It
     * holds the ClauseCounts file contents during a session.
     * 
     * @author		Gary Morris, Northern Virginia Community College
     *                   garymorris2245@verizon.net
     */
    public static class ClauseCounts extends TreeMap {
         ...
         */
        public static void loadFromDisk() throws JavaSystemException, FileNotFoundException {
    ...
    @author		Gary Morris, Northern Virginia Community College		garymorris2245@verizon.net
     */
    public static class ClauseIndex implements Serializable {
    ...
    public static class CB_EQC implements Serializable, Comparable, Comparator {
     ...
     */
    public static class CB_Ptr implements Comparable, Serializable {
...
//            cbEQC = getBaseEQC();  // Apparently not needed for all Ptrs. Cerated as needed
     ...
     */
    public static class BaseCB_Ptr extends CB_Ptr implements Comparable, Serializable {
    ...
    * @param    allowOverWrite  true when we are looking for a possible update
     ...
     * (EXCEPT LIBRARY). The file name will automatically be:
     * Directory-chosen-by-user + / +
     * Language name of this domain theory +
     * '(Adr)' IF it contains terms of address +  '.thy'
    ...
    public static void writeThyFile(DomainTheory dt) throws JavaSystemException, KSInternalErrorException {
        ...
        fc.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
        fc.setDialogTitle("Where to Write the Domain Theory");
        //  Display File-Open dialog and get User's selection
        ...
        while (tryAgain) {
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION) {
                ...
                if (dirPath.indexOf(libraryDirectory) > -1) {
                    msg = "You must store an exported Domain Theory\n"
                            + "outside the SILKin Library./nChoose a different location.";
                    JOptionPane.showMessageDialog(MainPane.desktop, msg, "Illegal Choice",
                            JOptionPane.ERROR_MESSAGE);
                } else { // a proper target directory was selected
                    try {
                        dt.comments = Context.convertDoubleQuotes(dt.ctxt.comments);
                        if (dt.addressTerms && dt.languageName.indexOf("(Adr)") < 0) {
                            dt.languageName += "(Adr)";
                        }
                        String fileName = dirPath + "/" + dt.languageName + ".thy";
                        PrintWriter outFile = new PrintWriter(new BufferedWriter(new FileWriter(fileName)));
                        dt.toThyFile(outFile);
                        System.out.println("Wrote: " + fileName);
                        msg = "Wrote " + fileName;
                        JOptionPane.showMessageDialog(MainPane.desktop, msg, "Exported Theory File",
                            JOptionPane.INFORMATION_MESSAGE);
                    } catch (IOException e) {
            ...
            } else { // end of APPROVE Option
                tryAgain = false;  // Don't try again, just return to calling method
            } // end of NOT APPROVED option
        } // end of while loop
     ...
     * They are too big; we'll read them on demand. THEREFORE, this method should 
     * NEVER be called on the contextUnderConstruction  --  it would wipe out
     ...
     * 
     * @param	fileName    a Context containing the current state of Data_Gathering
     * @param	makeStub    true = add this context to the ContextStub file.
     * 
     * @throws    FileNotFoundException  if the named file does not exist.
     * @throws    IOException           if there is a generic read/write failure.
     * @throws    ClassNotFoundException if a constructor cannot be found for a class stored in the file .
    ...
    public static void loadNewDomTh(File file) throws KSParsingErrorException, JavaSystemException, KSConstraintInconsistency,
        ...
        Linus dtLineServer = new Linus(file),
                macroLineServer = new Linus(libraryDirectory + "Standard_Macros");
        ParserDomainTheory parzer = new ParserDomainTheory(new Tokenizer(getDFA(), dtLineServer), new Tokenizer(getDFA(), macroLineServer));
        DomainTheory dt = parzer.parseDomainTheory(KinTermDef.standardMacros);
        addDomainTheory(dt);
    }

    /** Read in a .silk file, which is the primary output file of the 
     * KinshipEditor. That file will in turn point to a context and domain_theory 
     * file for a target culture. Use the contents of these files to create the 
     * appropriate structures for a learning context.
     * 
     ...
     * 
     * This solves an unlikely, but possible, problem. A multilingual person (or
     * team) might enter reason codes in more than one language. But they should
     * always display to the current User in her menu language. Hence, we translate.
     * 
     * @param ctxt  the context to be translated
     */
    public static void translateReasons(Context ctxt) {
        if (menuLanguage == null || screenElements == null) {
            return;  //  Don't know what language to translate into
        }
        ...
        }  //  OK. Now we have a valid list of language codes
        ArrayList<MyResBundle> elements = getElements();
        if (elements == null) {
            return;  //  give up
        }  //  And a list of ScreenElements bundles
        TreeMap<String, String> tmap = new TreeMap<String, String>();
        for (int i = 0; i < elements.size(); i++) {
            tmap.put(elements.get(i).getString("na"), "na");
            tmap.put(elements.get(i).getString("death"), "death");
            tmap.put(elements.get(i).getString("divorce"), "divorce");
            tmap.put(elements.get(i).getString("other"), "other");
        }  //  Now we can find the Key for the reason in every valid language
        for (Family fam : ctxt.familyCensus) {
            String reasonKey = tmap.get(fam.reason);
            if (reasonKey != null) {  
                //  ScreenElements is for the current menu language
                fam.reason = screenElements.getString(reasonKey);
            }            
        }
    }

    public static void getLangCodes() {
        String line, filename = resourceDirectory + "ValidLanguages.txt";
        BufferedReader file;
        try {
            file = new BufferedReader(new FileReader(filename));
        } catch (Exception exc) {
            // if this file doesn't exist, whole Library folder is corrupted. 
            return;
        }
        ...
        try {
            ...
            // if this fails, forget it.
            return;
        }
    }
    
    public static ArrayList<MyResBundle> getElements() {
        ArrayList<MyResBundle> elements = new ArrayList<MyResBundle>();
        for (String cod : langCodes) {
            Locale userLocale = new Locale(cod);
            MyResBundle se;
            try {
                se = MyResBundle.getBundle("ScreenElements", userLocale, null);
            } catch(Exception exc) {
                // if this fails, forget it.
                return null;
            }
...
//  Finally, write out the .ctxt file & update the disk stubs file.
     ...
     * 
     * @param fv    a FeatureVector whose elements are to be posted to the totals
     *              stored in static elements of the {@link FeatureVectorObj} class
     */ 
    ...
    <p> Do K-medioids clustering using distance of newbie to seed-or-medioid
