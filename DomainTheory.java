...
public class DomainTheory extends DT_Abstract2 {
    ...
    public TreeMap<String, String> getKTSymbols() {
        ArrayList<String> kTerms = new ArrayList<>(dyadsUndefined.keySet());
        kTerms.addAll(dyadsDefined.keySet());
        if (theory != null) {
            kTerms.addAll(theory.keySet());
        }
     ...
     * those definitions. Detect Synonyms & Umbrellas if possible.  For system-created auxiliary 
     * predicates (e.g. gen<aux>), ANY hits are noise. 
     ...
     */
    public ArrayList<Object> validateNewDyads(String kinTerm, int maxNoise) 
            throws KSBadHornClauseException, KSNoChainOfRelations2Alter,
...
//  if (kinTerm.equals("gagi_zhakka") || kinTerm.equals("nauwa_nani")) Context.breakpoint();
     ...
     * 
     * @param kinTerm the term whose definition has encountered data that doesn't fit
     * @param anomalies a list of all anomalies that we found.
     */
     ...
     * If exactly the same set of kin types share a few kin terms, we will
     * suggest to User that these are synonyms. If one term seems to cover a
     * number of kin types that collectively have a set of other kin terms,
     * we will suggest to User that it is an Umbrella term.
     * 
     * If the kin type in question is covered only by dyads that are under
     * challenge, skip that kin type.
     * 
     * @param kinTerm the term we are analyzing
     * @param prePos list of all positive dyads
     * @param preNeg list of all negative dyads
     * @param maxNoise the percent of misfits we can accept as 'normal noise'
     * @throws KSInternalErrorException 
     */
...
//  if (umb.equals("ancestor")) Context.breakpoint();
     ...
     *  make a Data Request.
     * 
     * @param kinTerm       the term we're trying to learn
     * @param ktMatches     the candidate matches
     * @param pos           all the positive matches
     * @param neg           all the negative matches
     * @param dataReqCounter a record of all the Data Requests for each kin term.
     *                       if we exceed a threshold, we'll stop asking User for 
     *                       data and take a guess. (Very rare!)
     * @throws KSInternalErrorException
...
//  testOutFile.println("\t" + kinTerm + "\t" + cbNonHits.size());
    ...
    /** This method will create a {@link ComposedDef} to be added to Suggestions.
     * 
     * @param kinTerm   the term we are trying to learn
     * @param octuple   a list of 8 things needed to make the composed definition.
     * @throws KSParsingErrorException
     * @throws JavaSystemException
     * @throws KSInternalErrorException
     * @throws KSConstraintInconsistency 
     ...
     * 
     * @param kinTerm   the term we're trying to learn
     * @param eqc       an equivalence class of definitions
     * @throws KSParsingErrorException
     * @throws JavaSystemException
     * @throws KSInternalErrorException
     * @throws KSConstraintInconsistency 
    ...
    /** Find all the BaseCBs that matches.
       <ol>
       <li>Find all BaseCBs which expand into the CBs of solidCBMatches or potFalseNeg.
       </li><li>
     Sort these into baseCB_Matches (an ArrayList of ArrayLists) as follows:
     <ol style="a">
     <li>Discard any BaseCB_Ptr whose KT-count (for the current MaxDist) is MUCH higher than # of KTs covered.
     </li><li>Using it's constituent expandedCBs, test against the dyads in POS & NEG, using fit().
     </li><li>Assign posScore = # of POS dyads covered; ditto for negScore.  Score = posScore - negScore.
     </li><li>If negScore > tolerance, discard this BaseCB_Ptr.  Otherwise, put it into baseCB_Matches[score].
     </li>
      </ol>
        </li><li>
        Make Pos2Cover = all pos dyads; make NegTokensCovered empty
      </li><li>
        For P = Nmbr-of-dyads-in-POS down to 1:
      <ol style="a">
        <li>In BaseCBMatches[P] pick the BaseCB_EQC with lowest negScore & put it into solution.
       </li><li>
        Remove all pos dysads covered from Pos2Cover.  Add all neg dyads covered to negTokensCovered.
       </li><li>
        If negTokensCovered has more than tolerance dyads, FAILURE.  No solution possible.
       </li><li>
        If pos2Cover is empty and negTokensCovered has less than tolerance, SUCCESS.  End loop.
       </li><li>
        If success, compose a KinTermDef which contains all the BaseCBs in Solution and return a Triple:
       <ul>
       <li>the composed KTD, 
       </li><li>
       list of languageNames used,
       </li><li>
       list of lang:kTerm:cbSeqNmbr of components.
       </li>
       </ul>
       </li>
       </ol>
...
//			levelsOfRecursion = oldLimit;
     ...
     * 
     * @param kinTerm   the term we're trying to learn
     * @param octuple   a list of 8 items needed to describe the anomaly, generated by
     *                  LearnedDT.learnKinTerm
     * @param tolerance a learning parameter
     * @param pos       the set of positive matches
     * @param neg       the set of negative matches
     * @return          an Anomaly object, ready for posting to the User
     * @throws KSInternalErrorException 
     */
     ...
     *  Success = true.
     * 
     * @param cb    the ClauseBody we are testing
     * @param dad   a dyad that we want to know if it fits this definition
...
//  System.out.println("### Found PotFalsePos with " + negHits + " negHits and " + posHits + " posHits.");
     ...
     *  Filter out the huge ones unlikely to be matches.
     * 
     * @param cbEQCs    the Library's ClauseIndex -- equivalence classes of ClauseBodies
     * @param pos       all the positive matches
     * @param cbCounts  the Library's ClauseCounts
     * @param sourceDT  the DomainTheory for which we are seeking definitions
     * @param maxDist   a learning parameter
     * @return          a TreeMap of candidates
     * @throws KSInternalErrorException
     * @throws FileNotFoundException
     * @throws JavaSystemException 
     */
