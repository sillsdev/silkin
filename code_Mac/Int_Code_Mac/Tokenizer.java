...
/**A Tokenizer consumes input from a {@link Linus} 1 character at a time,
 * building parse tokens as they are defined in a {@link DFA}. Beginning in
 * <code>currState</code> = 0, it consumes 1 character of input and consults the
 * DFA table.
 * <p>
 * For each non-empty cell in the row for currState, it reads the
 * character_class being sought and asks {@link JavaLex} if the current
 * character belongs to that class. The first 'true' ends the search. The next
 * state is read from that cell -&gt; <code>currState</code>. If the state =
 * -99, then a completed token is accepted (type is read from the state's
 * (row's) <code>tokenAccepted</code> field). Otherwise, go to the next state
...
//	else Context.breakFlag == false;
     ...
     * 
     * KAES included a variable "Reason" on each Marriage object: the reason for
     * the union to end. If the union was still current, the reason field would
     * contain 'N/A'.  This was tolerable in early versions of SILKin, but in the
     * later versions an '/' is illegal in a string.
     * 
     * To prevent an old SILK file from encountering a '/' and throwing an 
     * Exception, we inserted a check as each new line is read to see if it has
     * an old 'N/A'. If so, we change it to 'NA' before proceeding with the parse.
     */
    private TokenScanned please() throws KSParsingErrorException {
        ...
        if (charNum >= currentLine.length()) {	// if we've used up all of the line, get a new one            
            ...
            if (currentLine.contains("<reason>N/A</reason>")) {
                int w = currentLine.indexOf("N/A");
                currentLine = currentLine.substring(0, w) + "NA"
                        + currentLine.substring(w = 3);
            }
