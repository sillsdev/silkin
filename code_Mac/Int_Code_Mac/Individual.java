...
public boolean hasDoD() {
    ...
    */
    ...
    @param	root	String which must = "root"
    */
...
//  DON'T WRITE OUT THE LINKS LIST. IT WILL BE RE-CREATED FROM BACK-POINTERS IN THE LINKS.
    ...
    */
    String parseGender(String sex)	{
            if  (sex.equalsIgnoreCase("M") || sex.equalsIgnoreCase("Male")) return "M";
                ...
                //  Msgs for developer not translated.
    ...
    */
    void processNames(String nam)  {
        ...
        name = (firstNames.isEmpty() ? surname : firstNames + " " + surname);
   ...
   */
     ...
     * conforming value. Or, check that this String is a member of the value
     * field of the Constant arg. Type-checking has already been done, so we
     * know that arg takes values of type String Do not check a MathVar's
     * "contains" constraint -- that is done at higher level.
     *
     * @param stringObj the String object to be checked against these
     * constraints
     * @param arg a {@link MathVariable} containing constraints or a
     * {@link Constant}
     *
     * @return true if all constraints are satisfied (except 'contains')
     */
    public boolean checkProposedVal(String stringObj, Argument arg) {
        if (arg instanceof MathVariable) {
            MathVariable mathVar = (MathVariable) arg;
            String otherString;
            if ((mathVar.getVal() != null) && (mathVar.getVal().contains(stringObj))) {
                return true;
            }
            if (mathVar.notEqual != null) {
                Argument unEqArg;
                for (int i = 0; i < mathVar.notEqual.size(); i++) {
                    unEqArg = (Argument) mathVar.notEqual.get(i);
                    if (unEqArg.getVal() != null && unEqArg.getVal().contains(stringObj)) {
                        return false;
                    }
                }  //  end of loop thru notEqual
            }  //  end of notEqual-constraint-check
            if (mathVar.priorValues != null) {
                ...
                for (int i = 0; i < mathVar.priorValues.size(); i++) {
                    ...
                    if (priorFailure.equals(stringObj)) {
                        return false;
                    }
                }  //  end of loop thru priorValues
            }  //  end of priorValues-check
                ...
                for (int i = 0; i < mathVar.lessThan.size(); i++) {
                    ...
                    if (lessArg.getVal() != null) {
                        for (int j = 0; j < lessArg.getVal().size(); j++) {
                            ...
                            if (stringObj.compareTo(otherString) >= 0) {
                                return false;
                            }
            ...
            if (((Constant) arg).getVal().contains(stringObj)) {
                return true;
            } else {
                return false;
            }
