...
//  NOTE: Error messages intended for the developer or administrator are not translated.
    ...
    Gloss gloss;
    /**
     * An ArrayList<Object> of ClauseBodies. These clauses may contain any kind
     * of predicate, including previously-defined predicates in the local
     * language.
     */
    ArrayList<Object> definitions;

    /**
     * An ArrayList<Object> of ClauseBodies containing only primitive
     * predicates.
     */
    ArrayList<Object> expandedDefs;

    /**
     * An ArrayList<Object> of flags (strings) found between angle-brackets
     * within the kinTerm string.
     */
    ArrayList<Object> flags;

    /**
     * The exact equivalence classes (EQCs) to which this KTD's clauses belong.
     */
    TreeMap exactEQCs;

    /**
     * The structural equivalence classes (EQCs) to which this KTD's clauses
     * belong.
     */
    TreeMap structEQCs;
...
*/			}
     ...
     * <code>ClauseBody.structStr</code> the structural signature of each
     * clause.
     * 
	...
	public String toString(int limit)	{
     ...
     public ArrayList<String> toStringAList(String type, boolean inclPrim, boolean inclExt)	{
            ArrayList<String> list = new ArrayList<>();
            ClauseBody cb;
            String ch = clauseHead.toString();
            String pad, blankPad = "         ", nxt, line;
            pad = blankPad + " | ";
            blankPad += "    ";
            int nmbrDefs;
            if (type.equals("original"))  {
                cb = (ClauseBody)definitions.get(0);
                list.add(ch + " :- " + cb.toString());
                nmbrDefs = definitions.size(); 
                for (int i = 1; i < nmbrDefs; i++)  {
                    cb = (ClauseBody)definitions.get(i);
                    if (((cb.flags.contains("ext")) && (inclExt)) 
                        || ((! cb.flags.contains("ext")) && (inclPrim))) {
                        list.add(pad + cb.toString());                          
                    }  //  end of if-its-the-right-flavor
                }  // end of for-all-definitions
            }else {  //  type must be "expanded"  
                nmbrDefs = expandedDefs.size();
                if (nmbrDefs > 0)  {
                    for (int i = 0; i < nmbrDefs; i++)  {
                        ...
                        if (((cb.flags.contains("ext")) && (inclExt)) 
                            ...
                            line = cb.toString();
                            int where = line.indexOf("Lvl=");                            
                            list.add(nxt + pad + line.substring(0, where));  
                            line = line.substring(where);
                            if (line.length() < 96) {
                                list.add("    " + line);
                            } else {
                                where = line.indexOf("[Exp");
                                list.add("    " + line.substring(0, where));
                                list.add("    " + line.substring(where));
                            }
                        }     //  end of if-it's-the-right-flavor
                    }  // end of for-all-expanded-definitions
                }  // end of if-there-are-any-expandedDefs
            }  //  end of else-expanded-defs-are-selected
            return checkLineLengths(list, blankPad);
        }  // end of toStringAList method
     
     public ArrayList<String> checkLineLengths(ArrayList<String> input, String pad) {
         ...
         for (String line : input) {
             if (line.length() < 101) {
                 ...
                 if (line.indexOf("Ego") > -1) {  // 1st line of the ClauseBody
                     while (line.length() > 100) {
                        loc = line.indexOf("), ", 80) +2;  // the space
                        output.add(line.substring(0, loc));
                        line = pad + line.substring(loc);
                     }
                     output.add(line);
                 } else {  //  2nd or 3rd line of ClauseBody
                     while (line.length() > 100) {
                        int p3 = 80;
                        loc = line.indexOf(", ", p3) +1;  // the space
                        while (loc < 10) {
                            p3 -= 10;
                            loc = line.indexOf(", ", p3) +1;
                        }
                        output.add(line.substring(0, loc));
                        line = pad + line.substring(loc);
                     }
    ...
    */
     ...
     * 
     * @param	egoBag	contains at least 1 male and 1 female {@link Individual} 
     *                  who can serve as Ego in definitions.
     */
     ...
     * Only used when editing a definition.
     */
