 ...
 * Komogata's Compiler class (CIS 341) in Spring 2000 at Penn. It is NOT an
 * elegant or professional-quality parser -- just a utility to allow Domain
 * Theories written in Horn Clauses to be read into Kinship programs and
 * converted to the proper Java objects for reasoning there. The Context-Free
 * Grammar defining a Domain Theory is documented in
 * {@link DomainTheoryGrammar}.
 * <p>
 * NOTE: All exception text in this class is intended for the developer in
 * debugging,so it is not translated.
 * </p><p>
 * When constructed with a {@link Tokenizer}, a Parser will construct a Kinship
 * domain theory from the tokens found in the <code>Tokenizer's</code> input
 * file.
 * </p>
 *
...
public class ParserDomainTheory {
    ...
    public DomainTheory parseDomainTheory(String[] macros) 
            throws KSParsingErrorException, KSConstraintInconsistency {
        stdMacroPreds = macros;
        Arrays.sort(stdMacroPreds);
        return parseDomainTheory();
    }
    ...
    * DomainTheory -> LanguageName, HeaderOpts, StandardMacros, Theory.
    ...
    @throws 	<code>KSConstraintInconsistency</code>
     */
    public DomainTheory parseDomainTheory() 
            throws KSParsingErrorException, KSConstraintInconsistency {
    ...
    |  "comments", comma, string, rightParen.
    ...
    private void parseHdrOpts2() throws KSParsingErrorException {
        ...
        if (current.lexeme.equals("comments")) {
            scanner.readToken();  // consume the string 'comments'
            current = scanner.readToken();  // read next, which must be a comma
            if (!current.token.equals("comma")) {
                error("parseHdrOpts2 seeking a comma.");
            }
            ...
            if (!current.token.equals("string")) {
                ...
                dTheory.comments = current.lexeme;
            }
            current = scanner.readToken();  // read next, which must be a rightParen
            if (!current.token.equals("rightParen")) {
                error("parseHdrOpts2 seeking a rightParen.");
    ...
    /*U_D_Props -> comma, leftParen, starName, comma, "type", comma, Type, comma, 
    "single_value", comma, Boolean, UDP_Opts, rightParen, U_D_Props.
    |  \empty.
...
//   	 First: [symbol]	 Follow: [percent, leftCurly, $]
    private KinTermDef parseKinTermDef() throws KSParsingErrorException {
        ...
        if (stdMacroPreds != null) {
            dTheory.addTerm(ktd, stdMacroPreds);
        }else{
            dTheory.addTerm(ktd);
        }        
        ktd.domTh = dTheory;
