...
public class Learned_DT extends DomainTheory implements Serializable {
     ...
     * 
     * @param oldMap the current DyadTMap
     * @param c the releavant Context
     * @return a truncated DyadTMap
     */
...
//  if (kinTerm.equals("ancestor")) Context.breakpoint();
     ...
     * by the data for this kin term. <br/><br/>
     * 
     * If we find some candidate kin term definitions that are a solid match, then
     * we prepare a recommendation for the User: 
     *  -   we propose the clearly best definition, if there is one, or
     *  -   we suggest the additional data that would be most powerful in deciding among
     *      multiple candidates. <br/><br/>
     * 
     * If no solid candidates were found, then consider some 'noisy matches' -- 
     * definitions that would be a good fit if not for some misfit data. Based on a
     * noisy match, we may ask User to re-examine the data that doesn't match the pattern;
     * maybe it is noise (erroneous dyads). <br/><br/>
     * 
     * If both those methods fail, and the User has elected to try Sub-Pattern Matches,
     * then try to piece together a definition from component clauses of Library 
     * partial matches. <br/><br/>
     * 
     * Finally, if all three methods fail, try to induce a definition by pure induction.
     * <br/><br/>
     * In all cases, report any anomalies detected to User (but only ONCE) and always
     * check for synonyms and umbrellas. 
     * 
     * @param kinTerm the kin term to be learned
     * @param maxNoise percent of misfit data that can be questioned as possibly
     *                  erroneous. If more than this % don't match, they are assumed
     *                  to represent  a kinship fact of some kind.
     * @param ignorable percent of misfits data that can be ignored without correction
     * @param maxConf one half of the ignorable percent
     * @param sourceDT the domain theory that was cloned to make this learner
     ...
     * 
     * @param pos   POS is TreeMap&lt;KinType, ArrayList&lt;Dyad&gt;&gt;
     * @return      answer
     */
    public boolean containsChartableUDPs(TreeMap pos) {
        //  
