 ...
 * to particular Alter (or uses to address Alter). A node is stored in the 
 * appropriate row (Ego) and column (Alter) of the {@link KinTermMatrix} when
 ...
 * <p>
 * While a node is attached to an individual, its kin terms can be added, edited,
 * or deleted by the User via the Detail Display (when that individual is Alter).
 * </p>
 * 
...
public class Node implements Serializable {
    ...
    public String toExportString(String typ) {
        String out = "\t", temp;
        ArrayList<Object> terms = 
                (typ.equals("Reference") ? kinTermsRef
                : kinTermsAddr);
        if (! terms.isEmpty()) {
            out +=  (pcString.isEmpty() ? "" : pcString + " ");
            temp = terms.toString();
            out += (temp.equals("[Ego]") ? "    *" : temp);
        }
        terms = (typ.equals("Reference") ? extKinTermsRef
                : extKinTermsAddr);
        if (! terms.isEmpty()) {
            temp = terms.toString();
            out += (temp.equals("[Ego]") ? 
