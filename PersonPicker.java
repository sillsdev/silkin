...
/**
 * The PersonPicker is a window that displays a menu of {@link Individual}s.
 * When one person is selected, their fields are displayed to confirm it is the
 * right one. When the User clicks the "Choose" button, the Individual selected
 * is added to the appropriate field(s) of the <code>editor</code> which
 * launched this picker.
 * <p>
...
public class PersonPicker extends KSJInternalFrame {
    ...
    public PersonPicker(ArrayList<Object> srce, String title, String fieldNam,
        ...
        super(title);
        windowNum = se.getString("personPicker") + " " + fieldNam;
    ...
    public void buildEditor() {
        ...
        JLabel avaiLable = new JLabel(se.getString("personsAvailable") + " ");
        ...
        JLabel fullNameLabel = new JLabel(se.getString("fullName") + " ");
        fullName = new JLabel(ind.name);
        JLabel surnameLabel = new JLabel(se.getString("surname") + " ");
        ...
        birthday = new JLabel(se.getString("born") + " " + ind.getDateOfBirth());
        deathday = new JLabel(se.getString("died") + " " + ind.getDateOfDeath());
        gender = new JLabel(se.getString("gender") + " " + ind.gender);
        ...
        JLabel famLeft = new JLabel(se.getString("birthFamily") + " ");
        JPanel famRt = new JPanel();
        famRt.setLayout(new BoxLayout(famRt, BoxLayout.PAGE_AXIS));
        String husbName = se.getString("birthFamilyNotRecorded"), wifeName = "";
        ...
        JLabel marriageLabel = new JLabel(se.getString("marriages") + " ");
        ...
        String multiLine = se.getString("notMarried");
        ...
        if (!oneValue) {
            ...
            JLabel picks = new JLabel(se.getString("chosenSoFar") + " ");
    ...
    public class PPListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            if (e.getActionCommand().equals("pick person")) {
                ...
                String bday = "", dday = "";
                if (ind.hasDoB()) {
                    bday = ind.getDateOfBirth();
                }
                if (ind.hasDoD()) {
                    dday = ind.getDateOfDeath();
                }
                birthday.setText(se.getString("born") + " " + bday);
                deathday.setText(se.getString("died") + " " + dday);
                gender.setText(se.getString("gender") + " " + ind.gender);
                String husbName = se.getString("birthFamilyNotRecorded"), wifeName = "";
                if (ind.birthFamily != null) {
                    if (ind.birthFamily.husband != null) {
                        husbName = ind.birthFamily.husband.name;
                ...
                hName.setText(se.getString("husb") + " " + husbName);
                wName.setText(se.getString("wife") + " " + wifeName);
                String[] spice = PersonEditor.genSpouseMenu(ind, ind.marriages);
                String multiLine = "";
                for (int i = 0; i < spice.length; i++) {
                    if (i > 0) {
                ...
                if (selections.isEmpty()) {
                    ...
                    int choice = JOptionPane.showOptionDialog(p_picker,
                            se.getString("noSelections"),
                ...
                if (fieldName.equals("husband")) {  //  called by a FamEditor to pick a husband
                    ...
                    FamilyEditor fEd = new FamilyEditor(editor.ctxt, editor, 
                            se.getString("viewEditMarriage"),
                            bliss, "marriage");
                    fEd.desktop = desktop;
                ...
                try {
                    p_picker.setClosed(true);  //  close the panel
