...
public class DecisionFrame extends JFrame  implements HyperlinkListener {
    ...
    private void initComponents() {
        setTitle(se.getString("decisionsOnSuggestions"));
        setLayout(new BoxLayout(getContentPane(), BoxLayout.LINE_AXIS));

        //  Make right-hand display area
        jLabel1 = new JLabel();
        detailsScrollPane = new JScrollPane();
        jLabel1.setText(se.getString("details"));
        ...
        try {
            if (htmlFile != null) {
                ...
                String msg = se.getString("couldn'tFindFile") + " " + filePath;
                MainPane.displayError(msg, se.getString("cannotFindSuggestions"), 
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
        } catch (Exception e) {
            String msg = se.getString("failedURL") + " " + filePath + ".\n" + e;
            MainPane.displayError(msg, se.getString("cannotFindSuggestions"), JOptionPane.PLAIN_MESSAGE);
            return;
        }
    ...
    void makeComboModel() {
        Object[] suggArray = {se.getString("noSuggsError")};
        if (issues != null) {
            int size = DT_Abstract1.countLeaves(issues), cntr = 1;
            if (size > 0) {
                suggArray = new Object[size +1];
                suggArray[0] = se.getString("chooseSuggestion");
                ...
                while (suggIter.hasNext()) {
                    ...
                    for (Issue sug : sugLst) {
                        String dunMark = (sug.processed ? dun + ": " : "");
            ...
            if (suggestions.size() != size) {
                ...
                }  //  Msg for developer. No translation.
    ...
    public void markProcessed(int suggNmbr) {
        ...
        String newListing = listing.substring(0,dot) + dun + ": " + listing.substring(dot);
    ...
    public void markUnProcessed(int suggNmbr) {
        ...
        String newListing = listing.replace(dun + ":", "");
        model.removeElementAt(suggNmbr +1);
        updating = false;
        model.insertElementAt(newListing, suggNmbr +1);
        SIL_Edit.edWin.chart.dirty = true;
    ...
    void suggestionBoxActionPerformed() {
        ...
        } catch (Exception ex) { //  Msg for developer. No translation.
    ...
    public void hyperlinkUpdate(HyperlinkEvent event) {
        if (event.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
            ...
            } catch (IOException ex) {  //  Msg for developer. No translation.
